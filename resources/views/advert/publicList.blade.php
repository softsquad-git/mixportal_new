...
@section('content')
    <div class="container">
        <div class="page-ads">
            <form id="searchForm" action="{{ route('publicList') }}">
                <input type="hidden" name="type" value="{{ app('request')->input('type') }}"/>
                <input type="hidden" name="place_id" id="placeID"/>
                <div class="row">
                    <div class="col-xl-3 col-lg-3 col-md-3 col-sm-9 col-xs-9">
                        <input id="autocomplete" class="form-control" style="min-height: 40px" placeholder="{{ __('trans.city') }}" aria-label="City" type="text" size="50">
                    </div>
                    ...
                    <div class="col-xl-3 col-lg-3 col-md-3 col-sm-12 col-xs-12">
                        <select id="category" data-url="{{ route('api.categories.sub') }}" class="form-control" name="category"
                    ...
                    <div class="col-xl-1 col-lg-1 col-md-1 co-sm-12 col-xs-12">
                        <button class="btn btn-primary w-100" type="submit">
                            <img src="{{ asset('image/search.svg') }}" width="26" alt="search-icon"/>
                        </button>
                    </div>
                </div>
            </form>
            ...
            <div class="row">
                @foreach($data as $key => $item)
                    <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-xs-12">
                        <a href="{{ route('front.ads.show', ['id' => $item->id]) }}" style="display: block;background: url({{ $item->ad->getImage() }})" class="single-ad">
                            <div class="title">
                                {{ $item->title }}
                            </div>
                        </a>
